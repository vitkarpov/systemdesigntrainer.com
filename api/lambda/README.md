...
Diagnose and reset corrupted stream counters in Redis. This is useful when rate limiting issues occur due to counter drift from double-release bugs or other corruption.

**Use case:** When users are incorrectly blocked from starting conversations due to corrupted Redis stream counters.

**Payload:**
```typescript
{
  fix?: boolean;      // Optional: If true, actually resets counters. If false/omitted, dry-run mode (default: false)
  userId?: number;    // Optional: Specific user ID to check. If omitted, checks all users
}
```

**Example (dry-run - view counters without fixing):**
```bash
aws lambda invoke \
  --function-name sd-sim-production-admin \
  --cli-binary-format raw-in-base64-out \
...
**Example (fix corrupted counters):**
```bash
aws lambda invoke \
  --function-name sd-sim-production-admin \
  --cli-binary-format raw-in-base64-out \
...
**Counter issues detected:**
- `isNegative`: Counter is below 0 (indicates double-release bug)
- `isSuspicious`: Counter is above 2 (max allowed concurrent streams per user)
