...
vi.mock("@/api/hooks.gen", async (importOriginal) => {
  ...
  return {
    ...
    getSessionsControllerGetTranscriptQueryKey: vi.fn((id) => [
      "transcript",
      id,
    ]),
  };
...
const mockUseSessionsControllerGetSession =
  apiHooks.useSessionsControllerGetSession as ReturnType<typeof vi.fn>;
const mockUseSessionsControllerGetTranscript =
  apiHooks.useSessionsControllerGetTranscript as ReturnType<typeof vi.fn>;
const mockUseSessionsControllerGenerateFeedback =
  apiHooks.useSessionsControllerGenerateFeedback as ReturnType<typeof vi.fn>;
...
vi.mock("../../../hooks/useConversationStream", () => ({
  useConversationStream: (
    ...args: Parameters<typeof mockUseConversationStreamImpl>
  ) => mockUseConversationStreamImpl(...args),
}));
...
describe("Interview Page", () => {
  ...
  beforeEach(() => {
    ...
    mockUseSessionsControllerGetTranscript.mockReturnValue(
      mocks.mockTranscriptQuery,
    );
    mockUseSessionsControllerGenerateFeedback.mockReturnValue(
      mocks.mockGenerateFeedback,
    );
  ...
  describe("Rendering & Data Loading", () => {
    ...
    it("should poll session data when status is in_progress", () => {
      ...
      const sessionQueryConfig =
        mockUseSessionsControllerGetSession.mock.calls[0][1];
  ...
  describe("Message Sending & Streaming", () => {
    it("should send message with optimistic update", async () => {
      ...
      expect(mockSendMessage).toHaveBeenCalledWith("Hello interviewer", {
        nodes: [],
        edges: [],
      });
    });
    ...
    it("should display streaming text", () => {
      ...
      useStreamingStore
        .getState()
        .appendStreamText(123, "AI response in progress...");
  ...
  describe("Interview Lifecycle", () => {
    it("should end interview and navigate to feedback page", async () => {
      ...
      await waitFor(() => {
        expect(mocks.mockGenerateFeedbackMutate).toHaveBeenCalledWith({
          id: 123,
        });
    ...
    it("should show error toast if ending interview fails", async () => {
      const user = userEvent.setup();
      mocks.mockGenerateFeedbackMutate.mockRejectedValueOnce(
