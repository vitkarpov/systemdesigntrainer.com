...
describe('useStreamingStore', () => {
  ...
  describe('streaming lifecycle', () => {
    ...
    it('should complete streaming and reset state', () => {
      const { startStreaming, appendStreamText, completeStreaming } = useStreamingStore.getState()
      const abortController = new AbortController()

      startStreaming(sessionId, abortController)
      appendStreamText(sessionId, 'Test message')

      expect(useStreamingStore.getState().streams[sessionId]?.streamingText).toBe('Test message')
    ...
    it('should handle streaming error', () => {
      const { startStreaming, appendStreamText, errorStreaming } = useStreamingStore.getState()
      const abortController = new AbortController()
    ...
    it('should cancel streaming and abort request', () => {
      const { startStreaming, appendStreamText, cancelStreaming } = useStreamingStore.getState()
      const abortController = new AbortController()
  ...
  describe('selectors', () => {
    it('should get abort controller for session', () => {
      const { startStreaming, getAbortController } = useStreamingStore.getState()
      const abortController = new AbortController()
  ...
  describe('cleanup', () => {
    it('should clear stream for session', () => {
      const { startStreaming, appendStreamText, clearStream } = useStreamingStore.getState()
      const abortController = new AbortController()
