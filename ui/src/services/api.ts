...
export const getTranscript = async (sessionId: number): Promise<TranscriptMessage[]> => {
...
// AI endpoint
export const getAiResponse = async (sessionId: number): Promise<{ message: string }> => {
...
// Feedback endpoints
export const generateFeedback = async (sessionId: number): Promise<FeedbackReport> => {
  const response = await api.post(`/sessions/${sessionId}/feedback`);
  return response.data;
};

export const getFeedback = async (sessionId: number): Promise<FeedbackReport> => {
